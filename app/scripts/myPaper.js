...
function initializePath() {
	...
	path.segments = [];
	path.add([view.bounds.left,view.center.y]);
	for (var i = 1; i < points; i++) {
		var point = new Point(width / points * i, center.y);
		path.add(point);
	}
	path.add([view.bounds.right,view.center.y]);
	path.fullySelected = true;

	// path.add(new Point(view.bounds.bottomLeft))
	// path.add(new Point(view.bounds.bottomRight))
	// path.positionY =- 50
}